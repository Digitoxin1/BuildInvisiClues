...
var invisiClues = {
	...
	processTOC: function(main) {
		var tocElem = this.titleElement('Table of Contents', 'h2', false);
	...
	processSection: function(main, section, ulElem, upper) {
		if ('title' in section && section.title != '') {
			if (!('toc' in section)) {
				section.toc = true;
			}
			var title = section.title;
			var titleElem = this.titleElement(section.title, (section.toc ? 'h2' : 'h5'), section.toc, upper);
		...
		if ('subSections' in section) {
			for (var i = 0; i < section.subSections.length; i++) {
				...
				this.processSection(main, section.subSections[i], ulSubElem, false);
			}
		}
	},
	titleElement: function(text, type, setId, upper) {
		var elem = document.createElement(type);
		if (setId) {
